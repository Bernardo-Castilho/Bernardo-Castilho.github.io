...
const KEY_OPENAI = "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";
...
async function exportSheet(sheet, exportToJson) {
    ...
    let response = await fetch(`${URL_SHEETS}/values/'${sheet.title}'!A1:D?key=${KEY_SHEETS}`);
    if (!response.ok) {
        throw `** Skipping sheet [${sheet.title}] (no data?)`;
    }
    let json = await response.json();
    let values = json.values;
    ...
    for (let row = 1; row < values.length; row++) {
        ...
        if (embedding.length != DIM_PC) {
            throw `Embedding has wrong length (is ${embedding.length}, should be ${DIM_PC})`;
        }
